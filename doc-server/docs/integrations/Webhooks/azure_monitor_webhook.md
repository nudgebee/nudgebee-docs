...
```
https://<your-nudgebee-domain>/api/webhooks/azure-monitor?token=<generated-token>
```

Copy the URL exactly as the integration shows it. The path segment is the provider name — lowercase, no `_webhook` suffix — and nothing may follow it. `azure_monitor_webhook` is the integration type NudgeBee stores internally, not a URL you can post to; a URL that does not match the route above is answered by the NudgeBee web app with a 404 and never reaches the alert pipeline.
...
![The Azure Monitor Webhook tile in the Webhooks tab of the integrations catalog](../../../static/img/integrations/webhook-azure-monitor-card.png)
